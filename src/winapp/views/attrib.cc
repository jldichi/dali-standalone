...
*
* $Id: attrib.cc,v 1.3 2006/01/31 20:59:13 hardaiz Exp $
*
* DESCRIPTION: Implementation of AttributeZones class
*
*********************************************************************/
#ifdef __GNUC__
...
typedef AttributeBlock* pAtrBlk;
...
void AttributeZones::add(AttributeBlock &ae) 
{
	idCheck();
	...
	b_arr[j] = tr_new AttributeBlock(ae);
}

void AttributeZones::del(Int n) 
{
	idCheck();
...
Int AttributeZones::find(const AttributeBlock &ae) 
{
	idCheck();
...
void AttributeZones::reAlloc(Int n)
{
	idCheck();
...
static bool inZone(TextCursor &tc, TextCursor &s, TextCursor &e)
{
	return 	(s && e && tc >= s && tc <= e);
}

Int AttributeZones::getAttribute(TextCursor &tc, Int &lc)
{
	idCheck();
...
bool AttributeZones::setAttribute(TextCursor s, TextCursor &e, 
								  Int attr)
{
	idCheck();
	...
	// albertoi  AttributeBlock a_blk = s;
	TextCursor &S = s;        // albertoi
    AttributeBlock a_blk(S);  // albertoi
...
bool AttributeZones::delAttribute(TextCursor s, TextCursor &e, 
								  Int attr)
{
	idCheck();
...
bool AttributeZones::delAttribute(TextCursor &, Int )
{
	idCheck();
