...
struct fileVersion {
	String nm;
	String version;
	fileVersion(const String &name, const String &ver )
			: nm(name), version(ver) { }
};

static String lastVersLog		= NULL_STRING;

#include <ifound/parray.h>

declare(PtrArray,fileVersion);

static PtrArray(fileVersion) fvers;
...
{
	if (!fvers.isInRange(n)) return HIGH_VALUE;
	return fvers.elem(n)->nm;
}

static const void clearVersLog(const String &f)
	// Clears last verslog processed by Dali if necessary
{
	if (lastVersLog == f.field(0," "))
		lastVersLog = NULL_STRING;
}

bool VersionLog::redo()
{
	idCheck();
	DaliView *cview = cView.value();

	if (cview->isIStream() || cview->isPreserve())
		return false;

	String fileName = cview->getFileName();
	if (cview->isAdmin())
		fileName = fileName.field(0," ");

	Int n;
	if (fileName != lastVersLog) {
		n = fvers.dim();
		for (Int el =	0; el <	n; el++)
			tr_delete(fvers.elem(el));
		fvers.reAlloc(0);

		 // Tomo las versiones
		 String prsmsg,pipeline;
		 InputPipe prs(String("verslog ")+fileName);

		{ // message SCOPE
		 	WaitMsg pleaseWait(Editor::editorMsg("ONEMOMENT"),
		 					Editor::editorMsg("VersionLog"));
			pleaseWait.execute();

			while (prs >> pipeline) {
				String line;
				String ver = pipeline.field(0, "\t");
				line =	 String(VERS_WIDTH - ver.length(), ' ') + ver + " "
						+ String(pipeline.field(1, "\t"), DATE_WIDTH+1)
						+ String(pipeline.field(3, "\t"), USER_WIDTH+1)
						+ String(pipeline.field(4, "\t"), COMMENT_WIDTH+1);
				fvers.add(tr_new fileVersion(line,ver));
				prsmsg += pipeline;
			  };
		}
		if (prs.withError()) {
			 MsgBox errbox(libMsg("ERROR"), prsmsg, 9, 35, 5, 20);
			 errbox.execute();
			 return false;
		}
		else
			lastVersLog = fileName;
	}

	// Armo menu y tomo opcion
	n=fvers.dim();
	if (n > 0) {
		Int height = n;
		if (height > MAX_HEIGHT)
			height = MAX_HEIGHT;
		ScrollMenu sm(Editor::editorMsg("VersionLog")+" ("+toString(n)+")",
				fpReadLine, height, MAX_WIDTH+2);
		Int el = ERR;
		if ( sm.execute() != REJECT_EXIT && (el=sm.currOpt()) != ERR) {
			Editor::execAdminCommand (
					String("checkout -p -v ")+fvers.elem(el)->version+" "+fileName,
					fileName+" "+fvers.elem(el)->version
			);
		}
	}
	return false;
...
void VersionLog::undo()
{
	idCheck();
}

static void _undoCheckOut()
{
	DaliView *cview = cView.value();
	...
	// uncheckout del archivo
	String pipeline,errmsg;
	InputPipe uncheckout(String("checkout -u ")+cview->getFileName());
   	{ // message SCOPE
		WaitMsg pleaseWait(Editor::editorMsg("ONEMOMENT"),
							Editor::editorMsg("UnCheckOut"));
		pleaseWait.execute();

		 while(uncheckout >> pipeline)
			errmsg += pipeline;
	}

	// Si no hubo error hago reload del archivo
	if(uncheckout.withError()) {
		MsgBox errbox(libMsg("ERROR"),errmsg, 9, 35, 5, 20);
		errbox.execute();
	}
	else {
...
static void _undoCheckIn()
{
	...
	// uncheckout del archivo
	String pipeline,errmsg;
	InputPipe uncheckin(String("checkin -u ")+cview->getFileName());

	{ // message SCOPE
		WaitMsg pleaseWait(Editor::editorMsg("ONEMOMENT"),
							Editor::editorMsg("UnCheckIn"));
		pleaseWait.execute();

		while(uncheckin >> pipeline)
			errmsg += pipeline;
	}

	// Si no hubo error hago reload del archivo
	if(uncheckin.withError()) {
		MsgBox errbox(libMsg("ERROR"),errmsg, 9, 35, 5, 20);
		errbox.execute();
	}
	else {
		clearVersLog(cview->getFileName());
		cview->reload();
		cview->goCursor(inrow,atcol,true);
	}
}

void CheckOutFile::clear()
{
	idCheck();
	firstTime = true;
}

bool CheckOutFile::redo()
{
	idCheck();
	DaliView *cview = cView.value();

	// Es posible hacer un checkout de la view activa ?
	if (!cview->isRdOnly() || cview->isIStream() || cview->isLocked()) {
		Editor::beep();
		return false;
	}

	// Aviso de view modificado
	if (cview->isTextMod() && firstTime) {
		MLDBox dbox(libMsg("MESSAGE"),
				Editor::editorMsg("EXITRDONLY1")+NEW_LINE+
				Editor::editorMsg("EXITRDONLY3"),
				DBOX_YES);
		dbox.execute();
		if (dbox.getOpt() != DBOX_YES)
			return false;
	...
	// El nombre del archivo (por si contiene numero de version o esta adm)
	String fileName = cview->getFileName();
	if (cview->isAdmin() && fileName != fileName.field(0," ")) {
		MsgBox errbox(libMsg("ERROR"),Editor::editorMsg("NOTONVERFILE"), 6, 35, 5, 20);
		errbox.execute();
		return false;
	}
	...
	// Get del archivo
	String pipeline,errmsg;
	InputPipe checkout(String("checkout -e ")+fileName);

	{ // message SCOPE
		WaitMsg pleaseWait(Editor::editorMsg("ONEMOMENT"),
						Editor::editorMsg("CheckOut"));
			pleaseWait.execute();

		 while(checkout >> pipeline)
			errmsg += pipeline;
	}

	// Si no hubo error hago reload del archivo
	if(checkout.withError()) {
		Editor::beep();
		MsgBox errbox(libMsg("ERROR"),errmsg, 9, 35, 5, 20);
		errbox.execute();
	}
...
void CheckOutFile::undo()
{
	idCheck();
	_undoCheckOut();
}

bool UndoCheckOut::redo()
{
	idCheck();
	DaliView *cview = cView.value();
...
void UndoCheckOut::undo()
{
	idCheck();
}

void CheckInFile::clear()
{
	idCheck();
	firstComment = NULL_STRING;
}

bool CheckInFile::redo()
{
	idCheck();
	DaliView *cview = cView.value();

	// Es posible hacer un checkin del view activo ?
	if (cview->isRdOnly() || cview->isIStream()
		|| cview->isLocked() || cview->isAdmin() || cview->isPreserve()) {
		Editor::beep();
		return false;
	}

	// Pido comentario para el checkin
	String cmd;

	if (firstComment == NULL_STRING) {
		EdLineDialog dlog(Editor::adminComments,Editor::editorMsg("CheckIn"), "Comment : ",3,60,5,10);
		dlog.execute();
		if(dlog.isAccepted() != true)
			return false;     
			
		firstComment = cmd = dlog.value();
		*Editor::adminComments << cmd;
		//Editor::his->clear();
	}
	else
		cmd = firstComment;
	...
	// If view is not saved, save it now.
	if (cview->isTextMod()) {
		cview->store();
		cview->writeStLine();
	}
	...
	// Hago el checkin
	String errmsg,pipeline;
	InputPipe checkin(String("checkin -k -c '")+cmd+"' "+cview->getFileName());

	{ // message SCOPE
		WaitMsg pleaseWait(Editor::editorMsg("ONEMOMENT"),
						Editor::editorMsg("CheckIn"));
		pleaseWait.execute();

		while (checkin >> pipeline)
				errmsg += pipeline;
	}

	// Analizo resultado
	if (checkin.withError()) {
		MsgBox errbox(libMsg("ERROR"),errmsg, 9, 35, 5, 20);
		errbox.execute();
	}
	else {
...
void CheckInFile::undo()
{
	idCheck();
	_undoCheckIn();
}

bool UndoCheckIn::redo()
{
	idCheck();
	DaliView *cview = cView.value();

	// Es posible hacer un checkout de la view activa ?
	if (	cview->isAdmin()	|| cview->isIStream()
		 || cview->isLocked() || cview->isPreserve()) {
		Editor::beep();
		return false;
	}
...
void UndoCheckIn::undo()
{
	idCheck();
}

bool OpenLastVersion::redo()
{
	idCheck();
	DaliView *cview = cView.value();
	if(cview->isPreserve()) {
	...
	String fileName = cview->getFileName();
	if (cview->isIStream() || cview->isAdmin())
		 fileName = fileName.field(0," ");

	Editor::execAdminCommand(String("checkout -p ")+fileName,fileName+" "+
							 Editor::editorMsg("LastVersion"), false);

	return false;
}

void OpenLastVersion::undo()
{
	idCheck();
}

// end clase OpenLastVersion

bool DiffLastVersion::redo()
{
	idCheck();
	DaliView *cview = cView.value();
	if(cview->isPreserve()) {
